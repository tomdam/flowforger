# @flowforger/connectors-onedrive

This connector lets the [FlowForger engine](https://www.npmjs.com/package/@flowforger/engine) execute OneDrive for Business connector actions when running Power Automate flows locally — the same operations and parameter shapes Power Automate uses in the cloud, so a flow tested locally behaves the same after deployment.

It covers file and folder operations via Microsoft Graph and expects a Microsoft Graph token.

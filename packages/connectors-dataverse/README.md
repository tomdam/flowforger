# @flowforger/connectors-dataverse

This connector lets the [FlowForger engine](https://www.npmjs.com/package/@flowforger/engine) execute Microsoft Dataverse connector actions when running Power Automate flows locally — the same operations and parameter shapes Power Automate uses in the cloud, so a flow tested locally behaves the same after deployment.

It performs CRUD operations against the Dataverse Web API and expects an Azure AD token for your Dataverse environment (`https://<org>.crm.dynamics.com`).
